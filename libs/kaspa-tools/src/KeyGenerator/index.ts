import {Mnemonic, XPrv, PublicKeyGenerator, PublicKey} from 'kaspa-wasm-dev';
...
class KeyGenerator {
  ...
  public generateAddressFromXPrv(xprv: string): string {
    ...
    const privateKey = XPrv.fromXPrv(xprv);  // Convert string to XPrv
    ...
    const publicKey: PublicKey = keygen.receivePubkeys(0, 1)[0] as PublicKey; // Get first receive public key
