...
export class WithdrawalsRepository extends BaseRepository<WithdrawalEntity> {
  ...
  async setWaitingForKasStatus(orderId: string, session?: ClientSession): Promise<WithdrawalEntity> {
    return await this.transitionOrderStatus(orderId, WithdrawalStatus.WAITING_FOR_KAS, WithdrawalStatus.CREATED, {}, session);
  }

  async setCompletedStatus(_id: string, transactionId?: string): Promise<WithdrawalEntity> {
    return await this.transitionOrderStatus(_id, WithdrawalStatus.COMPLETED, WithdrawalStatus.CREATED, {
