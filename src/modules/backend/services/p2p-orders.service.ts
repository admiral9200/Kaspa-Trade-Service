...
export class P2pOrdersService {
    ...
    constructor (private readonly sellOrdersBookRepository: SellOrdersBookRepository){}

    async createSell(order: SellOrderDm) {
        try {
            const sellOrder =  await this.sellOrdersBookRepository.createSellOrder(order);
        } catch (err) {
